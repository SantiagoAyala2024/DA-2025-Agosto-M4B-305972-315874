...
async function submit(endPointUrl, urlEncodedData) {
    fetch(endPointUrl, {
        method: 'POST',
        headers: {
        ...
        if (status === 299) {
            try {
                ...
                await mostrarMensaje(text); //Por defecto...
            }
            return;
        } 
        try {
...
async function manejarError(status, text, url, data){
     try {
         ...
         procesarErrorSubmit(status,text);
                ...
                await mostrarMensaje("Se produjo un error, detalles en consola");
     }
}
